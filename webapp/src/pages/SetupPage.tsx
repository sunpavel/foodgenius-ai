...
export default function SetupPage() {
  ...
  useEffect(() => {
    fetch(`/api/user/me${getQueryUserId()}`, { headers: getHeaders() })
      ...
      .then((d) => {
        ...
        setMode(d?.onboardingDone ? 'profile' : 'onboarding');
      })
      .catch(() => setMode('onboarding'));
  }, []);

  // Нативная «Назад» Telegram: в профиле → выход в Меню; в мастере → шаг назад
  useEffect(() => {
    const bb = tg?.BackButton;
    if (!bb || !mode) return;
    if (mode === 'profile') {
      const cb = () => navigate('/calendar');
      bb.show(); bb.onClick(cb);
    ...
    bb.hide();
  }, [mode, step, tg, navigate]);

  function patch(p: Partial<UserPreferences>) {
    setPrefs((prev) => ({ ...prev, ...p }));
  }

  // Сохраняем профиль и запускаем генерацию в фоне → сразу на календарь (скелетон + поллинг)
  ...
  // ── Секции (используются и в мастере, и в едином профиле) ──────
  const goalBudgetSection = (
    <>
      <SectionTitle icon={<FlameIcon size={20} />}>Какая твоя цель?</SectionTitle>
      <div className="seg" style={{ marginBottom: 16 }}>
        {GOALS.map(({ v, icon, l }) => (
          <button key={v} onClick={() => patch({ goal: v, ...(v === 'maintain' ? { goalKg: undefined } : {}) })}
            className={`seg-item ${prefs.goal === v ? 'active' : ''}`} style={{ height: 76 }}>
            <span style={{ fontSize: 24 }}>{icon}</span>{l}
          </button>
        ))}
      </div>
      <AnimatePresence>
        {(prefs.goal === 'lose_weight' || prefs.goal === 'gain_muscle') && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ overflow: 'hidden', marginBottom: 16 }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--muted)', margin: '4px 0 10px' }}>На сколько кг?</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {GOAL_KG.map(({ l, v }) => (
                <button key={l} onClick={() => patch({ goalKg: v })} className={`chip ${prefs.goalKg === v ? 'active' : ''}`}>{l}</button>
              ))}
            </div>
          </motion.div>
        )}
      ...
      <div className="seg">
        {BUDGETS.map(({ v, l }) => (
          <button key={v} onClick={() => patch({ budgetLevel: v })} className={`seg-item ${prefs.budgetLevel === v ? 'active' : ''}`} style={{ height: 48 }}>{l}</button>
        ))}
      </div>
    </>
  );
  ...
  const dietSection = (
    <>
      <SectionTitle icon={<LeafIcon size={20} />}>Что исключаем?</SectionTitle>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
        <input value={customDiet} onChange={(e) => setCustomDiet(e.target.value)} placeholder="Другое (напр. без свинины)"
          style={{ flex: 1, height: 44, padding: '0 14px', borderRadius: 13, fontSize: 14, background: 'var(--card)', border: '1px solid var(--card-border)', color: 'var(--text)', outline: 'none' }}
          onKeyDown={(e) => { if (e.key === 'Enter' && customDiet.trim()) { patch({ dietaryRestrictions: [...prefs.dietaryRestrictions, customDiet.trim()] }); setCustomDiet(''); } }} />
        <button className="btn btn-ghost" style={{ width: 56 }}
          onClick={() => { if (customDiet.trim()) { patch({ dietaryRestrictions: [...prefs.dietaryRestrictions, customDiet.trim()] }); setCustomDiet(''); } }}><PlusIcon size={18} /></button>
      </div>
      <button className="btn btn-ghost" onClick={() => patch({ dietaryRestrictions: [] })}>Нет ограничений</button>
    </>
  );

  const activitySection = (
    <>
      <SectionTitle icon={<FlameIcon size={20} />}>Насколько ты активен?</SectionTitle>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {ACTIVITY.map(({ v, l, hint }) => {
          const active = prefs.activityLevel === v;
          return (
            <button key={v} onClick={() => patch({ activityLevel: v })}
              style={{ display: 'flex', alignItems: 'center', gap: 12, width: '100%', height: 56, padding: '0 14px', borderRadius: 13, cursor: 'pointer', textAlign: 'left', border: active ? '1px solid var(--accent)' : '1px solid var(--card-border)', background: active ? 'rgba(52,211,153,0.10)' : 'rgba(255,255,255,0.02)', boxShadow: active ? '0 0 20px -6px var(--glow)' : 'none', transition: 'all 0.18s' }}>
              <span style={{ width: 20, height: 20, borderRadius: '50%', flexShrink: 0, border: active ? 'none' : '1.5px solid var(--faint)', background: active ? 'var(--gradient)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#06241c' }}>{active && <CheckIcon size={12} strokeWidth={3} />}</span>
              <span style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                {DAYS.map(({ code, label }) => (
                  <button key={code} onClick={() => patch({ activityDays: toggle(prefs.activityDays ?? [], code) })}
                    className={`chip ${(prefs.activityDays ?? []).includes(code) ? 'active' : ''}`} style={{ flex: 1, padding: 0 }}>{label}</button>
                ))}
              </div>
  ...
  const portionsSection = (
    <>
      <SectionTitle icon={<UsersIcon size={20} />}>На сколько человек готовим?</SectionTitle>
  ...
  // ── РЕЖИМ «ПРОФИЛЬ»: все настройки на одном экране ────────────
  if (mode === 'profile') {
    return (
      <div style={{ padding: '20px 16px 0' }}>
        <h1 style={{ margin: '0 0 4px', fontSize: 26, fontWeight: 800, letterSpacing: '-0.02em' }}>Профиль</h1>
        <p style={{ color: 'var(--muted)', fontSize: 13, margin: '0 0 22px' }}>Изменили настройки — нажмите сохранить, и меню пересоберётся под них</p>

        <div style={{ marginBottom: 26 }}>{goalBudgetSection}</div>
        <div style={{ marginBottom: 26 }}>{dietSection}</div>
        <div style={{ marginBottom: 26 }}>{activitySection}</div>
        <div style={{ marginBottom: 26 }}>{portionsSection}</div>

        <button className="btn btn-primary" disabled={busy} onClick={handleSave}>
          {busy ? 'Сохраняю...' : '💾 Сохранить и обновить меню'}
        </button>
        {phase === 'error' && (
          <p style={{ textAlign: 'center', color: 'var(--rose)', fontSize: 13, margin: '12px 0 0' }}>Не получилось — попробуйте ещё раз</p>
        )}
        <div style={{ height: 16 }} />
      </div>
    );
  ...
  // ── РЕЖИМ ОНБОРДИНГА: мастер из 4 шагов ───────────────────────
  const canNext = step === 0 ? Boolean(prefs.goal) : true;
  const steps = [goalBudgetSection, dietSection, activitySection, portionsSection];

  return (
    <div style={{ padding: '20px 16px 0' }}>
      <div style={{ display: 'flex', gap: 6, marginBottom: 24, padding: '0 4px' }}>
        {[0, 1, 2, 3].map((i) => (
          <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i <= step ? 'var(--gradient)' : 'rgba(255,255,255,0.1)', transition: 'background 0.3s' }} />
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={step} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.2 }}>
          {steps[step]}
          {step === 3 && (
            <div className="glass" style={{ padding: 16, marginTop: 16, background: 'rgba(52,211,153,0.05)', borderColor: 'rgba(52,211,153,0.18)' }}>
              ...
              <div style={{ fontSize: 13, lineHeight: 1.7, color: 'var(--text)' }}>
                ...
                {prefs.activityLevel !== 'none' && (<><br />Спорт: {ACTIVITY.find((a) => a.v === prefs.activityLevel)?.l.toLowerCase()}, {(prefs.activityDays ?? []).length} дн./нед</>)}
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 24, marginBottom: 16 }}>
        ...
        {step < 3 ? (
          <button className="btn btn-primary" style={{ flex: 2, opacity: canNext ? 1 : 0.5 }} disabled={!canNext} onClick={() => setStep((s) => s + 1)}>Далее →</button>
        ) : (
          <button className="btn btn-primary" style={{ flex: 2 }} disabled={busy} onClick={handleSave}>{busy ? 'Сохраняю...' : '✨ Создать меню'}</button>
        )}
      </div>

      {phase === 'error' && (
        <p style={{ textAlign: 'center', color: 'var(--rose)', fontSize: 13, margin: '0 0 16px' }}>Не получилось — попробуйте ещё раз или отправьте /plan боту</p>
      )}
    </div>
  );
}
